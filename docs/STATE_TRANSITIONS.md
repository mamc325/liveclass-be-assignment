...
- 동시성 흐름 (별도 정리) → `docs/CONCURRENCY.md`
...
>
> 또한 본 SQL의 `now()`는 **의사 표현**이며, 실제 구현에서는 애플리케이션 `Clock`에서 생성한 `OffsetDateTime`을 쿼리 파라미터(`:now`)로 전달한다. 이는 취소 가능 기간 경계 테스트에서 `Clock` mock으로 시각을 주입하기 위함이다. 상세 정책은 `docs/CONCURRENCY.md` 6.4 참조.
>
> native UPDATE 후 영속성 컨텍스트의 stale entity 문제는 `@Modifying(clearAutomatically=true, flushAutomatically=true)` + 변경 후 fresh select로 처리한다. 상세는 `docs/CONCURRENCY.md` 6.5 참조.
...
- [ ] `now()`는 서비스 빈에 주입된 `Clock`에서 `OffsetDateTime`을 생성해 **쿼리 파라미터(`:now`)로 전달**한다. DB의 `now()`를 직접 호출하지 않는다. (테스트에서 시각 mock 가능)
- [ ] native UPDATE는 `@Modifying(clearAutomatically=true, flushAutomatically=true)` 사용. 변경 후 응답 DTO는 fresh select로 구성해 stale entity 회피.
